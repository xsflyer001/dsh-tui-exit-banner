 ...
 */
export function readPendingSession(startedAtMs: number): string | undefined {
    try {
        const sessionId = readFileSync(RESUME_FILE, 'utf8').trim()
        if (!sessionId) return undefined
        if (statSync(RESUME_FILE).mtimeMs < startedAtMs) return undefined
        return sessionId
    }
    catch {
        return undefined
 ...
 */
export function readSessionTitle(sessionId: string, untitled: string): string {
    try {
        const index = JSON.parse(readFileSync(INDEX_FILE, 'utf8')) as {
            entries?: Record<string, { derived?: { title?: unknown } }>
        }
