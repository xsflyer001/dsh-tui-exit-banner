 ...
 */
export function isExitReceipt(text: string): boolean {
    if (!text.includes(UPSTREAM_HINT)) return false
 ...
 */
export function isSuppressNotice(text: string): boolean {
 ...
 */
export function commandFromReceipt(text: string, sessionId: string): string {
...
type State = {
    /** 插件 apply 的时刻，用于判定 resume.txt 是否为这一轮所写。 */
    startedAt: number
    /** 已经渲染好、等 exit 钩子打印的横幅。 */
    pending?: string
    /** 本次退出交给 restart / update / crash 流程，不要补横幅。 */
    suppress: boolean
    /** 配置覆盖的恢复命令；空串表示从上游回执里提取。 */
    commandOverride: string
    /** 生成恢复命令用的启动器别名。 */
    launcher: string
    /** 渲染入口（闭包捕获了本次 apply 的配置）。 */
    build: (sessionId: string, command: string) => string
    /** 被包装前的 stdout.write，dispose 时还原。 */
...
export function apply(ctx: Context, config: ExitBannerConfig = {}): void {
    const resolved = {
        logo: (config.logo ?? 'shadow') as LogoStyle,
    ...
    const build = (sessionId: string, command: string): string => renderBanner({
        sessionId,
    ...
    const patchedWrite = function patchedWrite(this: unknown, ...args: unknown[]): boolean {
        const chunk = args[0]
        ...
        if (text !== undefined) {
            if (isSuppressNotice(text)) {
                ...
                if (sessionId !== undefined) {
                    ...
                    const at = text.indexOf(UPSTREAM_HINT)
                    return (originalWrite as (...a: unknown[]) => boolean).call(this, text.slice(0, at), ...args.slice(1))
                }
            }
        }
    ...
    process.stdout.write = patchedWrite as unknown as typeof process.stdout.write
